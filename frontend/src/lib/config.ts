...
async function fetchConfig(): Promise<AppConfig> {
  ...
  // If runtimeApiUrl is empty string (Tailscale domain), use relative paths via Next.js rewrites
  if (runtimeApiUrl === '') {
    console.log('🔧 [Config] Using relative paths (Tailscale domain detected)')
    // Use relative path to fetch backend config (Next.js rewrites will proxy to internal API)
    try {
      ...
      if (response.ok) {
        ...
        config = {
          apiUrl: '', // Empty string = use relative paths (Next.js rewrites will proxy)
        ...
        console.log('✅ [Config] Successfully loaded API config via relative path:', config)
        return config
      } else {
        console.log('⚠️ [Config] Backend config endpoint returned status:', response.status)
      }
    } catch (error) {
      console.log('⚠️ [Config] Could not fetch config via relative path:', error)
    }
    // Fallback to default config with empty apiUrl
    config = {
      apiUrl: '',
      version: 'unknown',
      buildTime: BUILD_TIME,
    ...
    console.log('⚠️ [Config] Using fallback config with relative paths')
    return config
  }

 ...
 * - Mixed: [source:x, source_insight:z]
 *
...
export function parseSourceReferences(text: string): ParsedReference[] {
  // Match pattern: (source_insight|source):alphanumeric_id
 ...
 * - After commas: [source:a, source_insight:b] → each converted separately
...
export function convertReferencesToMarkdownLinks(text: string): string {
  ...
  const refPattern = /(source_insight|source):([a-zA-Z0-9_]+)/g
  ...
  while ((match = refPattern.exec(text)) !== null) {
    ...
    const validTypes = ['source', 'source_insight']
 ...
 *
 * @param text - Original text with references
 * @param titleMap - Optional map from reference key (e.g., "source:abc") to display title
 * @returns Text with numbered citations and reference list appended
 *
 * @example
 * Input: "See [source:abc] and [source_insight:xyz]. Also [source:abc] again."
 * Output: "See [1] and [2]. Also [1] again.\n\nReferences:\n[1] - [Alpha zero](#ref-source-abc)\n[2] - [source_insight:xyz](#ref-source_insight-xyz)"
 */
export function convertReferencesToCompactMarkdown(
  ...
  // Helper function to get title from titleMap
  const getTitle = (key: string): string | null => {
    if (!titleMap) return null
    if (titleMap instanceof Map) {
      return titleMap.get(key) || null
    }
    return titleMap[key] || null
  }
  ...
  for (const [, refData] of referenceMap) {
    ...
    // Use title if available, otherwise fall back to original format
    const displayText = title || `${refData.type}:${refData.id}`

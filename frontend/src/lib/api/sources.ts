...
export const sourcesApi = {
  ...
  create: async (data: CreateSourceRequest & { file?: File }) => {
    ...
    formData.append('build_pageindex', String(data.build_pageindex ?? false))
  ...
  buildPageIndex: async (sourceId: string) => {
    const response = await apiClient.post<{
      status: string
      message: string
      source_id: string
      source_title?: string
    }>(`/sources/${sourceId}/build-pageindex`)
    return response.data
  },

  buildPageIndexForNotebook: async (notebookId: string) => {
    const response = await apiClient.post<{
      status: string
      message: string
      notebook_id: string
    ...
    }>(`/notebooks/${notebookId}/build-pageindex`)
    return response.data
  },
}

...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (hostHeader) {
      ...
      // Check if this is a Tailscale domain - use relative paths via Next.js rewrites
      // This allows API calls to go through Next.js proxy to container-internal API
      // Works for Tailscale Serve, port forwarding, and other scenarios where only frontend port is exposed
      if (hostname.endsWith('.ts.net')) {
        console.log(`[runtime-config] Detected Tailscale domain (${hostname}), using relative paths via Next.js rewrites`)
        return NextResponse.json({
          apiUrl: '', // Empty string means use relative paths (Next.js rewrites will proxy to internal API)
        })
      }

      // For other domains, construct the API URL with port 5055

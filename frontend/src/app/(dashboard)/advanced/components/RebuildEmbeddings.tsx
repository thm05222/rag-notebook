...
export function RebuildEmbeddings() {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (statusData.status === 'completed' || statusData.status === 'failed') {
          ...
          // Log detailed error information if failed
          if (statusData.status === 'failed') {
            console.error('Rebuild operation failed:', {
              commandId: cmdId,
  ...
  const isAnyTypeSelected = includeSources || includeInsights
              ...
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  <div className="space-y-1">
                    <p className="font-medium">Failed to start rebuild</p>
                    <p className="text-sm">
                      {(rebuildMutation.error as Error)?.message || 
                       (rebuildMutation.error && typeof rebuildMutation.error === 'object' && 'response' in rebuildMutation.error && 
                        rebuildMutation.error.response && typeof rebuildMutation.error.response === 'object' && 
                        'data' in rebuildMutation.error.response && rebuildMutation.error.response.data && 
                        typeof rebuildMutation.error.response.data === 'object' && 'detail' in rebuildMutation.error.response.data
                        ? String(rebuildMutation.error.response.data.detail)
                        : 'Unknown error')}
                    </p>
                    {rebuildMutation.error && typeof rebuildMutation.error === 'object' && 'response' in rebuildMutation.error && (
                      <details className="text-xs mt-2">
                        ...
                        <pre className="mt-1 p-2 bg-muted rounded overflow-auto">
                          {JSON.stringify(rebuildMutation.error, null, 2)}
                        </pre>
                      </details>
                    )}
                  </div>
          ...
          <div className="space-y-4">
            ...
            {status.error_message && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  <div className="space-y-2">
                    <p className="font-medium">Rebuild Operation Failed</p>
                    <p>{status.error_message}</p>
                    {status.stats && status.stats.failed !== undefined && status.stats.failed > 0 && (
                      <p className="text-sm text-muted-foreground">
                        {status.stats.failed} item(s) failed to process.
                      </p>
                    )}
                  </div>
                </AlertDescription>
          ...
          <AccordionItem value="time">
            ...
            <AccordionContent className="space-y-2 text-sm">
              ...
              <ul className="list-disc list-inside space-y-1 ml-2">
                ...
                <li><strong>Sources:</strong> Slower than insights (creates multiple chunks per source)</li>
              </ul>

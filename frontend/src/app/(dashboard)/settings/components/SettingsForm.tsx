...
import { DefaultContextModeToggle, DefaultContextMode } from '@/components/settings/DefaultContextModeToggle'
...
const settingsSchema = z.object({
  ...
  default_context_mode: z.enum(['full', 'off', 'insights']).optional(),
})
...
export function SettingsForm() {
  ...
  const {
    ...
    defaultValues: {
      ...
      default_context_mode: undefined,
    }
  })
  ...
  useEffect(() => {
    if (settings && !hasResetForm) {
      const formData = {
        default_content_processing_engine_doc: settings.default_content_processing_engine_doc as 'auto' | 'docling' | 'simple' | undefined,
        default_content_processing_engine_url: settings.default_content_processing_engine_url as 'auto' | 'firecrawl' | 'jina' | 'simple' | undefined,
        default_embedding_option: settings.default_embedding_option as 'ask' | 'always' | 'never' | undefined,
        auto_delete_files: settings.auto_delete_files as 'yes' | 'no' | undefined,
        default_context_mode: settings.default_context_mode as 'full' | 'off' | 'insights' | undefined,
      }
      reset(formData)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Collapsible open={expandedSections.files} onOpenChange={() => toggleSection('files')}>
              ...
              <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                <p>Once your files are uploaded and processed, they are not required anymore. Most users should allow the system to delete uploaded files from the upload folder automatically. Choose <strong>no</strong>, ONLY if you are using the notebook as the primary storage location for those files (which you shouldn&apos;t be at all). This option will soon be deprecated in favor of always downloading the files.</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Notebook Context</CardTitle>
          <CardDescription>
            Configure default context mode for sources in notebooks
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Controller
              name="default_context_mode"
              control={control}
              render={({ field }) => (
                <div className="space-y-2">
                  <DefaultContextModeToggle
                    mode={(field.value as DefaultContextMode) || 'full'}
                    onChange={(mode) => {
                      setValue('default_context_mode', mode, { shouldDirty: true })
                    }}
                  />
                  <Collapsible open={expandedSections.context} onOpenChange={() => toggleSection('context')}>
                    <CollapsibleTrigger className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                      <ChevronDownIcon className={`h-4 w-4 transition-transform ${expandedSections.context ? 'rotate-180' : ''}`} />
                      Help me choose
                    </CollapsibleTrigger>
                    <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                      <p>• <strong>Not included</strong>: Sources will not be included in chat context by default. You can manually enable them per source.</p>
                      <p>• <strong>Insights only</strong>: Only insights generated from sources will be used in chat context by default. Useful for faster, more focused conversations.</p>
                      <p>• <strong>Full content</strong> (recommended): Full source content will be used in chat context by default. Provides the most comprehensive context for AI responses.</p>
                      <p>Note: This is the default setting for new sources. You can override this setting for individual sources in each notebook.</p>
                    </CollapsibleContent>
                  </Collapsible>
                </div>
              )}

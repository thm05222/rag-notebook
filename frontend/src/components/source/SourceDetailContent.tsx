...
export function SourceDetailContent({
  ...
  const [deleteInsightDialog, setDeleteInsightDialog] = useState<{ open: boolean; insight: SourceInsightResponse | null }>({
    open: false,
    insight: null
  })
  ...
  const handleDeleteInsight = async () => {
    const insight = deleteInsightDialog.insight
    if (!insight) return

    const deletedInsightId = insight.id

    try {
      setDeletingInsight(true)
      await insightsApi.delete(insight.id)
      toast.success('Insight deleted successfully')
      setDeleteInsightDialog({ open: false, insight: null })
      
      // If the deleted insight is currently selected, clear selection but keep dialog open
      if (selectedInsight?.id === deletedInsightId) {
        setKeepDialogOpen(true)
        setSelectedInsight(null)
        // Reset keepDialogOpen after a short delay to allow dialog to update
        setTimeout(() => setKeepDialogOpen(false), 100)
      }
      
      await fetchInsights()
      // Refresh source to update insights_count
      await fetchSource()
    } catch (err) {
  ...
  const handleEmbedContent = async () => {
    ...
    try {
      ...
      let errorMessage = 'Failed to embed content'
      if (isAxiosError(err)) {
  ...
  return (
    <div className="flex flex-col h-full">
                      ...
                      <div key={insight.id} className="rounded-lg border bg-background p-4">
                        ...
                        <div className="mt-3 flex justify-end gap-2">
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={() => setDeleteInsightDialog({ open: true, insight })}
                          >
      ...
      <SourceInsightDialog
        open={Boolean(selectedInsight) || keepDialogOpen}
        onOpenChange={(open) => {
          if (!open && !keepDialogOpen) {
            setSelectedInsight(null)
          }
        }}
      ...
      <ConfirmDialog
        open={deleteInsightDialog.open}
        onOpenChange={(open) => setDeleteInsightDialog({ open, insight: deleteInsightDialog.insight })}
        title="Delete Insight"
        description={`Are you sure you want to delete this insight? This action cannot be undone and will remove it from both the database and vector search.`}
        confirmText="Delete"

...
class AgenticAskRequest(BaseModel):
    question: str = Field(..., description="Question to ask the knowledge base")
    decision_model: Optional[str] = Field(None, description="Model ID for decision making")
    evaluation_model: Optional[str] = Field(None, description="Model ID for evaluation")
    synthesis_model: Optional[str] = Field(None, description="Model ID for answer synthesis")
    max_iterations: Optional[int] = Field(10, description="Maximum number of iterations", ge=1, le=20)
    max_tokens: Optional[int] = Field(50000, description="Maximum token usage", ge=1000, le=200000)
    max_duration: Optional[float] = Field(300.0, description="Maximum execution time in seconds", ge=30.0, le=3600.0)
...
class ModelCreate(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Model type (language, embedding)",
    )
...
class SettingsResponse(BaseModel):
    ...
    default_context_mode: Optional[str] = None
...
class SourceCreate(BaseModel):
    ...
    build_pageindex: bool = Field(
        False, description="Whether to build PageIndex for this source"
    )

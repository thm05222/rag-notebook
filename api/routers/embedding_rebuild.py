    ...
    try:
        ...
        if request.include_sources:
            if request.mode == "existing":
                # Count sources with embeddings from Qdrant
                try:
                    ...
                    if client:
                        ...
                        total_estimate += len(unique_sources)
                except Exception as e:
                    logger.warning(f"Failed to count sources from Qdrant: {e}")
            else:
...
async def get_rebuild_status(command_id: str):
    ...
    - **stats**: breakdown by type (sources, insights, failed)
    ...
    try:
        ...
        if status.result and isinstance(status.result, dict):
            ...
            # Build stats - handle both success and failed cases
            # Even if command failed, we might have partial progress
            ...
            logger.debug(f"Command result structure for {command_id}: {list(result.keys())}")
        ...
        # Add error message if failed - extract from multiple possible locations
        if status.status == "failed":
            ...
            if status.result:
                if isinstance(status.result, dict):
                    # Try error_message field first
                    error_message = status.result.get("error_message")
                    # If not found, try checking if result itself is an error
                    if not error_message and "error" in status.result:
                        error_message = str(status.result.get("error"))
                    # Check for exception messages
                    if not error_message and "exception" in status.result:
                        error_message = str(status.result.get("exception"))
                elif isinstance(status.result, str):
                    # Result might be a string error message
                    error_message = status.result
            
            # Try status.error_message attribute if available
            if not error_message and hasattr(status, "error_message") and status.error_message:
                error_message = str(status.error_message)
            
            # Log the status structure for debugging
            logger.debug(f"Command status for {command_id}: status={status.status}, result_type={type(status.result)}, result={status.result}")
            logger.debug(f"Status attributes: {dir(status)}")
            
            if error_message:
                response.error_message = error_message
            else:
                # Provide a default message with context
                response.error_message = f"Rebuild operation failed. Status: {status.status}. Check logs for details."
                logger.warning(f"Failed rebuild command {command_id} but no error message found in status.result or status.error_message")

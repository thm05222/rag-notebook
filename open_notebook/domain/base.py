...
class ObjectModel(BaseModel):
    ...
    async def get(cls: Type[T], id: str) -> T:
        ...
        try:
            ...
            table_name = id.split(":")[0] if ":" in id else None
            
            # If ID has no prefix and we're calling from a specific class, use class table_name
            if table_name is None and cls.table_name:
                table_name = cls.table_name
                # Construct full ID with prefix
                full_id = f"{cls.table_name}:{id}"
            elif table_name is None:
                raise InvalidInputError(f"ID {id} has no table prefix and class {cls.__name__} has no table_name")
            else:
                full_id = id
            ...
            result = await repo_query("SELECT * FROM $id", {"id": ensure_record_id(full_id)})
            if result:
                return target_class(**result[0])
            else:
                raise NotFoundError(f"{table_name} with id {full_id} not found")
        except Exception as e:
